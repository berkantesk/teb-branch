...
public class Board {
    ...
    public Vector<Student> getBoardFollowers(int boardID) throws InstantiationException, IllegalAccessException, SQLException {
        Vector<Student> boardFollowers = new Vector<Student>();
        ResultSet rs = null;
        Connection dbConn = PostgresConnection.getConnection();
        try {
            dbConn.setAutoCommit(false);
            callStmt = dbConn.prepareCall("{? = call alex.pkgBoardOperations.getBoardFollowers(?)}");
            callStmt.registerOutParameter(1, java.sql.Types.OTHER);
            callStmt.setInt(2, boardID);
            callStmt.execute();
            rs = (ResultSet) callStmt.getObject(1);
    ...
    public Vector<Tag> getBoardTags(int boardID) throws InstantiationException, IllegalAccessException, SQLException {
        Vector<Tag> boardTags = new Vector<Tag>();
        ResultSet rs = null;
        Connection dbConn = PostgresConnection.getConnection();
        try {
            dbConn.setAutoCommit(false);
            callStmt = dbConn.prepareCall("{? = call alex.pkgBoardOperations.getBoardTags(?)}");
            callStmt.registerOutParameter(1, java.sql.Types.OTHER);
            callStmt.setInt(2, boardID);
            callStmt.execute();
            rs = (ResultSet) callStmt.getObject(1);
